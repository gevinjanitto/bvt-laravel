...
<<<<<<< HEAD
          @foreach([['instagram','Instagram'],['facebook','Facebook'],['youtube','YouTube'],['tiktok','TikTok']] as [$key,$label])
            @if(!empty($social[$key]))<a href="{{ safe_link($social[$key]) }}" target="_blank" rel="noopener noreferrer" aria-label="{{ $label }}" title="{{ $label }}" data-testid="footer-social-{{ $key }}" class="w-9 h-9 rounded-full bg-white/10 hover:bg-brand flex items-center justify-center transition-colors">{!! fa_icon($key) !!}</a>
            @else<span aria-label="{{ $label }} belum diatur" title="{{ $label }} belum diatur" data-testid="footer-social-{{ $key }}-unset" class="w-9 h-9 rounded-full bg-white/10 text-white/50 flex items-center justify-center">{!! fa_icon($key) !!}</span>@endif
          @endforeach
        </div>
      </div>
      <div class="lg:col-span-2"><h3 class="text-xs tracking-widest font-bold uppercase mb-5" data-testid="footer-menu-title">{{ $F['menuTitle'] }}</h3><ul class="space-y-2.5 text-sm text-white/75">@foreach(blocks('navigation') as $i => $l)<li><a href="{{ safe_link($l['to']) }}" data-testid="footer-menu-{{ $i }}" class="hover:text-white transition-colors">{{ $l['label'] }}</a></li>@endforeach</ul></div>
      <div class="lg:col-span-3"><h3 class="text-xs tracking-widest font-bold uppercase mb-5" data-testid="footer-services-title">{{ $F['servicesTitle'] }}</h3><ul class="space-y-2.5 text-sm text-white/75">@foreach($F['services'] as $i => $l)<li><a href="{{ safe_link($l['url']) }}" data-testid="footer-service-{{ $i }}" class="hover:text-white transition-colors">{{ $l['label'] }}</a></li>@endforeach</ul></div>
      <div class="lg:col-span-3 min-w-0"><h3 class="text-xs tracking-widest font-bold uppercase mb-5" data-testid="footer-contact-title">{{ $F['contactTitle'] }}</h3><p class="text-sm font-semibold" data-testid="footer-legal">{{ $brand['legal'] }}</p>
        <ul class="space-y-3 text-sm text-white/75 mt-4">
          <li><a href="{{ wa_url() }}" target="_blank" rel="noopener" data-testid="footer-contact-whatsapp" class="flex items-start gap-2 hover:text-white transition-colors">{!! fa_icon('whatsapp', 'w-4 h-4 text-brand shrink-0 mt-0.5') !!}<span class="break-words min-w-0">+{{ $contact['whatsapp'] }}</span></a></li>
=======
...
<<<<<<< HEAD
        <p data-testid="footer-copyright">© {{ date('Y') }} {{ $brand['name'] }}. All rights reserved. | Design &amp; Develop <a href="https://www.maiharta.com" target="_blank" rel="noopener noreferrer" class="text-white/85 hover:text-white underline-offset-4 hover:underline" data-testid="footer-maiharta-link">CV Maiharta</a></p>
        <span class="text-white/30">|</span>
        <a href="{{ route('admin.login') }}" class="hover:text-white transition-colors" data-testid="footer-admin-link">Admin</a>
      </div>
      <div class="flex flex-wrap items-center gap-2"><span data-testid="footer-payment-title">{{ $F['paymentTitle'] }}</span>@foreach($F['payments'] as $i => $p)<span class="rounded-md bg-white/10 px-2.5 py-1 text-[10px] font-bold text-white/85" data-testid="footer-payment-{{ $i }}">{{ $p }}</span>@endforeach</div>
=======
